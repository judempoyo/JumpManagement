                ...
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        <tr>
                            ...
                            <td class="px-3 py-1 whitespace-nowrap">{{ number_format($item->unit_price, 2, ',', ' ') }} $</td>
                            <td class="px-3 py-1 whitespace-nowrap">{{ $item->quantity }}</td>
                            <td class="px-3 py-1 whitespace-nowrap">{{ number_format($item->subtotal, 2, ',', ' ') }} $</td>
                        </tr>
                        @endforeach
                    </tbody>
                </table>
            ...
            <div class="bg-gray-50 p-4 rounded-lg">
                <table class="w-full">
                    <tr class="border-b border-gray-200">
                        ...
                        <td class="py-2 text-right font-medium">{{ number_format($purchaseOrder->total, 2, ',', ' ') }} $</td>
                    </tr>
                    <tr class="border-b border-gray-200">
                        <td class="py-2 text-gray-600">Remise:</td>
                        <td class="py-2 text-right font-medium">{{ number_format($purchaseOrder->discount, 2, ',', ' ') }} $</td>
                    </tr>
                    <tr class="border-b border-gray-200">
                        <td class="py-2 text-gray-600">Montant à payer:</td>
                        <td class="py-2 text-right font-bold">{{ number_format($purchaseOrder->amount_payable, 2, ',', ' ') }} $</td>
                    </tr>
                    <tr>
                        <td class="py-2 text-gray-600">Statut:</td>
                        <td class="py-2 text-right">
                            <span class="px-2 py-1 rounded-full text-xs font-medium {{ $purchaseOrder->paid ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800' }}">
